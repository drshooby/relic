...
**Vite 8 + React 19.2 + TypeScript 6**, `src/` layout. It is a pure SPA — no server, no SSR — because every design decision points that way: all fetching is client-side, and the build is a static bundle that talks only to the read API.

- Package manager is **bun**. Not npm, not pnpm.
- CSS Modules. No Tailwind.
- `@/*` maps to `dashboard/src/`, so components import as `@/components/CustomComponent`. The alias is declared **twice** — `paths` in `tsconfig.app.json` for the typechecker, `resolve.alias` in `vite.config.ts` for the bundler. Changing one without the other breaks the build in a confusing way.
- **Component layout — one directory per component, no exceptions:**

  ```
  src/components/
    CustomComponent/
      CustomComponent.tsx      export function CustomComponent
      CustomComponent.module.css
      index.tsx                export { CustomComponent } from "./CustomComponent";
  ```

  The `index.tsx` re-export is what makes `@/components/CustomComponent` work without a file suffix. Every component follows this, including one-off children.
- `lib/` and `types.ts` live at `src/`, not inside `components/` — they are not components.
- Two tsconfig settings bite: **`verbatimModuleSyntax`** means a type import without `import type` is a compile error, and **`noUnusedLocals`** means a leftover import fails `bun run build`.
- Vite's dev server is **port 5173**. The API's CORS allowlist must match it, or every browser request fails with an opaque CORS error while `curl` keeps working.
- Client-side env vars must be prefixed **`VITE_`** (e.g. `VITE_RELIC_API_URL`) or Vite silently omits them from the bundle — undefined at runtime, no error.
- Tests are **Vitest**, configured inside `vite.config.ts` so the `@/*` alias is defined once and shared. `bun run test` is non-watching; `bun run test:watch` watches.