...
Read [dashboard/AGENTS.md](dashboard/AGENTS.md) first — **this Next.js is not the one you know.** It is 16.3 with React 19.2, and APIs, conventions, and file structure differ from older versions. Consult `dashboard/node_modules/next/dist/docs/` before writing components; do not write from memory.

- Package manager is **bun** (`bun.lock`, `packageManager: bun@1.3.14`). Not npm, not pnpm.
- App Router, TypeScript strict, CSS Modules. No Tailwind.
- `@/*` maps to the `dashboard/` root, so components import as `@/components/CustomComponent`.
...
- The `<!-- BEGIN:nextjs-agent-rules -->` block in `AGENTS.md` is regenerated by `next dev`. Removing it from a diff only recreates it; commit it with your work to keep the tree clean.