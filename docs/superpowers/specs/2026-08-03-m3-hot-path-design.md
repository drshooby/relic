...
- **`game_time_s` is stored as `Decimal`, not a Python float.** DynamoDB's Number type is exact decimal, so `240.623` round-trips without IEEE754 error — but boto3's resource layer refuses to guess a float's intended precision and raises `TypeError: Float types are not supported. Use Decimal types instead.` The conversion must go through the string form: `Decimal(str(240.623))` is exactly `Decimal("240.623")`, while `Decimal(240.623)` reproduces the binary value `240.62299999999999044...` and reintroduces precisely the error this is meant to avoid. `game_time_s` is the EEG alignment key, so the precision is load-bearing.
